...
stochastic_motor = StochasticSolidMotor(
    solid_motor=BuildingSolidMotor.solid_motor,
...
stochastic_tail = StochasticTail(
    tail=BuildingRocket.tail, top_radius=0.0001, bottom_radius=0.0001, length=0.0001
)

stochastic_rail_buttons = StochasticRailButtons(
    rail_buttons=BuildingRocket.rail_buttons, buttons_distance=0.01
)

stochastic_main = StochasticParachute(parachute=BuildingRocket.main, cd_s=0.1, lag=0.1)

stochastic_drogue = StochasticParachute(
    parachute=BuildingRocket.drogue, cd_s=0.07, lag=0.1
)
...
test_dispersion = MonteCarlo(
    filename="Rocketpy/Enterprise/Monte_Carlo_results/monte_carlos_analysis_output.txt",
...
test_dispersion.export_ellipses_to_kml(
    filename="Rocketpy/Enterprise/Monte_Carlo_results/monte_carlo_class_example.kml",
    origin_lon=DefiningEnvironment.env.longitude,
    origin_lat=DefiningEnvironment.env.latitude,
    type="impact",
)
